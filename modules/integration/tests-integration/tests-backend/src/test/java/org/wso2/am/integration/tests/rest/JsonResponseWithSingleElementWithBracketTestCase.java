...
/**
 * This test case is for validating Json response with a single
 * element with brackets are received in order through API Manager
 * when response caching is enabled
 */
public class JsonResponseWithSingleElementWithBracketTestCase extends APIMIntegrationBaseTest {
    ...
    public JsonResponseWithSingleElementWithBracketTestCase(TestUserMode userMode) {
    ...
    public void setEnvironment() throws Exception {
        ...
        if (TestUserMode.SUPER_TENANT_ADMIN == userMode) {
            loadSynapseConfigurationFromClasspath("artifacts" + File.separator + "AM"
                            ...
                            + File.separator + "single_element_json_return_api.xml",
    ...
    @Test(groups = {"wso2.am"}, description = "test if single element with brackets json response is processed " +
            "correctly")
    public void testAPIInvocation() throws Exception {
        ...
        HttpRequestUtil.doGet(
                getAPIInvocationURLHttp(apiContext + "/1.0.0/backend"), requestHeaders);
        ...
        assertTrue(serviceResponse.getData().contains("[\"water\"]"),
